...
int VideoPlayer::InitDecoder()
{
	...
	hr = CreateMediaSource(fileUrl, &m_pSource);
	if (FAILED(hr)) { std::cerr << "Couldn't create Media Source. Error " << hr << std::endl; return hr; }

	//Create the media session.
	hr = MFCreateMediaSession(NULL, &m_pSession);
	if (FAILED(hr)) { std::cerr << "Couldn't create Media Session. Error " << hr << std::endl; return hr; }
...
HRESULT VideoPlayer::OpenFileAndCreateSession(LPCWSTR sURL)
{
	if (!sURL)
	{
		return E_INVALIDARG;
